...
for jj=1:ct.N_mic
    ...
    for ii=0:ct.M_th
        ...
        var.Hprim = 1i^(ii)*(Bessel_sph(ii,k.*Antenna.Rmicro(ii+1))-Bessel_sph_1_deriv(ii,k.*ct.r_micsph)/Hankel_sph_1_deriv(ii,ct.hankel_order,k.*ct.r_micsph)*Hankel_sph(ii,ct.hankel_order,k.*Antenna.Rmicro(ii+1)));
        var.Bessel_int = 1i^(ii)*(Bessel_sph(ii,k.*Antenna.Rmicro(ii+1)));
        if ii==0;
