...
% [ HData,t, k ] = SphmicFfrDataMeas( OutSig,EntrySig,System)
% Function used to extract data from measurements with microphone array
...
if N.N_meas<N.N_sweep
    OutSig=[OutSig ;zeros(N.N_sweep-N.N_meas,50)];
    fprintf('%i zeros have been added to the measurement data \n', N.N_sweep-N.N_meas)
else if N.N_meas>N.N_sweep
        EntrySig=[EntrySig ;zeros(N.N_sweep-N.N_meas,50)];
...
%% Average data
% Select frequency
[~,HData,~,~,t ] = FrfSystemFinal_data(OutSig,EntrySig,N,ct,ct.fc);
