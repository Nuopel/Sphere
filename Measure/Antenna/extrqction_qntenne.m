...
var.select_freq=[250 500 1000 1500 2000];
close all
...
subplot(131)
grid_mat=fliplr(reshape(H_antenna_target.h_sig_fft(pos,:),size(Antenna.Y_mat)));
...
grid_mat_decode=fliplr(reshape(H_antenna_decode.h_sig_fft(pos,:),size(Antenna.Y_mat)));
...
caxis(var.cax);
  ct.theta_simu=wavefrontarrow( grid_mat_decode,Antenna,'r' );
    wavefrontarrow( grid_mat,Antenna );
...
grid_mat_simu=fliplr(reshape(H_antenna_simu.h_sig_fft(pos,:),size(Antenna.Y_mat)));
...
Pmes_mat = reshape(erreur ,size(Antenna.X_mat)) ;
    ...
    set(hfigc, 'LineWidth',1.0,'Color', [1 1 0]);
caxis(var.cax);
   ct.theta_simu=wavefrontarrow( grid_mat_simu,Antenna,'r' );
    wavefrontarrow(grid_mat,Antenna );
end
