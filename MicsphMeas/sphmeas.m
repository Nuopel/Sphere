...
[data.EntrySig,ct.Fs]=audioread('../../DataMicSph18_05/sweep_signal_mic_sph.wav');
[data.OutSig,ct.Fs_sca]=audioread('../../DataMicSph18_05/test_hp14.w64');
...
data.OutSig=bsxfun(@times,data.OutSig,data.calib.');
...
for ii=1:4
 subplot(2,2,ii)
ct.M=ii+1;
[Pressure.p_recons, ~ ] = Pressure_map_SphMic(ct.M,Bmn.recons(1:var.m_sum_vect(ii+2),1),ct,N,var,Antenna);title(sprintf('Reconstruction sphere mic calib 1000-%i',ii+1));
