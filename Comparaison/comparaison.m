...
if isfield(data, 'sc_simu')==0
    ...
    [data.sc_simu_virt ,H_simu_virt,~,N,ct,~] = simu_impulse( data.hp_simu, data.sweep,Antenna,ct,N);
...
ct.micn=28;
comp3( H_simu.h_sig(:,ct.micn),H_reel.h_sig(:,ct.micn),H_virt.h_sig(:,ct.micn),t.h_sig,1 )
...
delay.vs=mean(delay.simu_moy-delay.virt_moy);

comp3( H_norm.h_simu_virt(:,ct.micn),H_norm.h_reel(:,ct.micn),H_norm.h_virt(:,ct.micn),t.h_sig,2)
plot_micro( delay.simu,delay.reel,delay.virt,3)

%% Repropagation
% [ H_norm.h_simu_repro] = Repropagation(delay.reel-delay.simu,H_norm.h_simu,N);
% [data.sc_simu] = Repropagation(delay.reel-delay.virt,data.sc_simu,N);
% [~, delay.simu_repro]=max(H_norm.h_simu_repro);
...
shading interp
view([0 90]);xlim([1 8]);xlabel('Microphone');ylabel('Delay in sample ');
...
set(gca,'XTick',1:2:8);set(gca,'XTickLabel',25:2:32);
title('Simulated virtual source')
...
title(text.freq(ii));
xlabel('Microphone');ylabel('Erreur');
ylim([0 500]);
pause(0.1);
end
%%
...
semilogx(a)
axis([100 2000 0 100]);
xlabel('Frequency');ylabel('Mean erreur frequency');

subplot(212)
a=mean(erreur.value');
plot(a)
xlabel('Microphone');ylabel('Mean erreur mic');
%%
...
text.b='Measured real source';text.a='Measured virtual source';text.c='Erreur relative';
texts.b='Simulated real source';texts.a='Simulated virtual source';texts.c='Erreur relative';texts.freq=text.freq;
